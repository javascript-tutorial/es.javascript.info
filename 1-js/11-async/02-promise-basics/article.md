...
- `state` - inicialmente `"pendiente"`, que luego cambia a `"cumplido"` cuando se llama a `resolve` o a `"rechazado"` cuando se llama a `reject`.
- `result` - inicialmente `undefined`, que luego cambia a `valor` cuando se llama a `resolve(valor)` o a `error` cuando se llama a `reject(error)`.
