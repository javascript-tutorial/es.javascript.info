...
Esto tenía el beneficio de nunca romper código existente, pero lo malo era que cualquier error o decisión imperfecta realizada por los creadores de JavaScript se quedó para siempre en el lenguage.
...
Volviendo arriba, tengamos en cuenta que `"use strict"` se puede poner al inicio de la mayoría de tipos de funciones en lugar del script entero. De esta manera, se activa el modo estricto únicamente en esa función. Pero, normalmente, la gente lo utiliza para el script entero.
...
A partir de ahora, cuando utilices la consola del navegador para probar cosas, ten en cuenta que no utiliza `use strict` por defecto.

En ocasiones, cuando `use strict` causa una diferencia, obtendrás resultados incorrectos.

Incluso si pulsamos `key:Shift+Enter` para escribir varias líneas y ponemos `use strict` al principio, no funcionará. Esto es por cómo la consola ejecuta el código internamente.
