...
Podemos rastrear múltiples dedos en contacto usando su `pointerId`. Cuando el usuario se mueve y luego quita un dedo, obtenemos eventos `pointermove` y `pointerup` con el mismo `pointerId` que teníamos en `pointerdown`.
