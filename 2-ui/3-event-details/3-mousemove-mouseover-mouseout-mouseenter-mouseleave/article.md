...
El evento `mouseover`  se produce cuando un puntero del mouse aparece sobre un elemento y `mouseout` -- cuando se va.

![](mouseover-mouseout.svg)

Estos eventos son especiales porque tienen propiedades `relatedTarget`. Esta propiedad complementa `target`. Cuando el puntero del mouse deja un elemento por otro, uno de ellos se convierte en `target` y el otro en - `relatedTarget`.
...
```online
En el siguiente ejemplo, cada cara y sus características son elementos separados. Cuando mueves el mouse, puedes ver los eventos del mouse que ocurren en el área de texto.

Cada evento tiene la información sobre ambos `target` y `relatedTarget`:
...
El navegador verifica la posición del mouse de vez en cuando. Y si nota cambios entonces activa los eventos.
...
Eso es bueno para el rendimiento porque puede haber muchos elementos intermedios. Realmente no queremos procesarlo todo dentro y fuera de cada uno.
...
Este HTML tiene dos elementos: el `<div id="hijo">` está adentro del `<div id="padre">`. Si mueves el mouse rápidamente sobre ellos entonces probablemente solo el `div` hijo active los eventos, o probablemente el padre, o probablemente no ocurran eventos en lo absoluto.

También mueva el cursor hacia el `div` hijo y luego muévalo rápidamente hacia abajo a través del padre. Si el movimiento es lo suficientemente rápido entonces el padre será ignorado. El mouse cruzará el elemento padre sin notarlo.
...
El evento `mouseover` sobre un elemento anidado brota. Entonces si `#parent` tiene el cotrolador `mouseover`, se activa:
...
1. `mouseout [target: parent]` (dejo al padre), luego
2. `mouseover [target: child]` (vino hacia el hijo, brotó).

[codetabs height=360 src="mouseoverout-child"]
```

Como se muestra, cuando el puntero se mueve del elemento `#parent` a `#child`, los dos controladores se activan en el elemento padre: `mouseout` y `mouseover`:
...
**Si no examinamos `event.target` dentro de los controladores podría parecer que el puntero del  elemento `#parent` y volvió a él inmediatamente.**

Pero ese no es el caso. El puntero aún está sobre el elemento padre, simplemente se adentró más en el elemento hijo.

Si hay algunas acciones al abandonar el elemento padre,por ejemplo: una animación se ejecuta con `parent.onmouseout`, usualmente no la queremos cuando el puntero se adentre más sobre `#parent`.

Para evitar esto lo que podemos hacer es checar `relatedTarget` en el controlador y si el mouse aún permancece dentro del eelmento entonces ignorar dicho evento.
...
1. Las transiciones hacía/desde los descendientes no se cuentan.
...
Los eventos `mouseenter/leave` son muy simples de usar. Pero no brotan por sí solos. Por lo tanto no podemos usar la delagación de eventos con ellos.
...
En nuestro caso nos gustarías manejar las transiciones entre las celdas de la tabla `<td>`: entrando a una celda y saliendo de ella. Otras transiciones, como dentro de una celda o fuera de cualquiera de ellas no nos interesan. Vamos a filtrarlas.

Esto es lo que podemos hacer:

- Recordar el elemento `<td>` resaltado actualmente en una variable, llamémosla `currentElem`.
- En `mouseover` -- ignorar el evento si permanecemos dentro del `<td>` actual.
- En `mouseout` -- ignorar el evento si no hemos dejado el `<td>` actual.
...
Intenta mover el cursor dentro y fuera de las celdas de la tabla y dentro de ellas. Rápido o lento -- no importa --. Solo se ilumina `<td>` como un todo, a diferiencia del ejemplo anterior.
```
...
- Los eventos `mouseover/out` y `mouseenter/leave` tienen una propiedad adicional: `relatedTarget`. Es el elemento de dónde venimos o hacia dónde vamos, complementario con `target`.
